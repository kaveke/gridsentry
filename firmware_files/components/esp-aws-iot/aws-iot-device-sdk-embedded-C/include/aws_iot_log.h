...
#define IOT_DEBUG(...)    \
	{\
	ESP_LOGI(TAG, "DEBUG:   %s L#%d ", __func__, __LINE__);  \
...
#define FUNC_ENTRY    \
	{\
	ESP_LOGI(TAG, "FUNC_ENTRY:   %s L#%d ", __func__, __LINE__);  \
	}
#define FUNC_EXIT    \
	{\
	ESP_LOGI(TAG, "FUNC_EXIT:   %s L#%d ", __func__, __LINE__);  \
	}
#define FUNC_EXIT_RC(x)    \
	{\
	ESP_LOGI(TAG, "FUNC_EXIT:   %s L#%d Return Code : %d ", __func__, __LINE__, x);  \
	return x; \
	}
#else
#define FUNC_ENTRY
...
#define IOT_WARN(...)   \
	{ \
	ESP_LOGI(TAG, "WARN:  %s L#%d ", __func__, __LINE__);  \
	ESP_LOGI(TAG, __VA_ARGS__); \
	ESP_LOGI(TAG, ""); \
	}
#else
...
#define IOT_ERROR(...)  \
	{ \
	ESP_LOGE(TAG, "ERROR: %s L#%d ", __func__, __LINE__); \
	ESP_LOGI(TAG, __VA_ARGS__); \
	ESP_LOGI(TAG, ""); \
	}
#else
