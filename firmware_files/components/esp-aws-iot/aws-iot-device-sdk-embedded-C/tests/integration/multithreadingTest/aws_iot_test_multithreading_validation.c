...
static void *aws_iot_mqtt_tests_publish_thread_runner(void *ptr) {
	...
	for(itr = 0; itr < PUBLISH_COUNT; itr++) {
		snESP_LOGI(TAG, cPayload, 100, "%s_Thread : %d, Msg : %d", AWS_IOT_MY_THING_NAME, threadId, itr);
		ESP_LOGI(TAG, "\nMsg being published: %s ", cPayload);
...
static void *aws_iot_mqtt_tests_sub_unsub_thread_runner(void *ptr) {
	...
	AWS_IoT_Client *pClient = (AWS_IoT_Client *) ptr;
	char testTopic[50];
	snESP_LOGI(TAG, testTopic, 50, "%s_temp", INTEGRATION_TEST_TOPIC);
...
int aws_iot_mqtt_tests_multi_threading_validation() {
	...
	do {
		...
		ESP_LOGI(TAG, "\nFinished thread count : %d ", finishedThreadCount);
		sleep(1);
	...
	if(RX_RECEIVE_PERCENTAGE <= percentOfRxMsg  && 0 == rxMsgBufferTooBigCounter && 0 == rxUnexpectedNumberCounter &&
	   0 == wrongYieldCount) {
		...
		ESP_LOGI(TAG, "Published Messages: %d , Received Messages: %d ", PUBLISH_COUNT * MAX_PUB_THREAD_COUNT, rxMsgCount);
		ESP_LOGI(TAG, "QoS 1 re publish count %d", rePublishCount);
		ESP_LOGI(TAG, "Connection Attempts %d", connectCounter);
		ESP_LOGE(TAG, "Yield count without error during callback %d", wrongYieldCount);
		test_result = 0;
	} else {
...
int main() {
	ESP_LOGI(TAG, "\n");
	ESP_LOGI(TAG, "******************************************************************");
	ESP_LOGI(TAG, "* Starting MQTT Version 3.1.1 Multithreading Validation Test     *");
	ESP_LOGI(TAG, "******************************************************************");
	int rc = aws_iot_mqtt_tests_multi_threading_validation();
	if(0 != rc) {
		ESP_LOGI(TAG, "\n*******************************************************************");
		ESP_LOGE(TAG, "*MQTT Version 3.1.1 Multithreading Validation Test FAILED! RC : %d ", rc);
		ESP_LOGI(TAG, "*******************************************************************");
		return 1;
	}

	ESP_LOGI(TAG, "******************************************************************");
	ESP_LOGI(TAG, "* MQTT Version 3.1.1 Multithreading Validation Test SUCCESS!!    *");
	ESP_LOGI(TAG, "******************************************************************");
