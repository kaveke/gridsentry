...
static void aws_iot_mqtt_tests_message_aggregator(AWS_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
	...
	if(10 >= params->payloadLen) {
		snESP_LOGI(TAG, tempBuf, params->payloadLen, params->payload);
...
static void *aws_iot_mqtt_tests_publish_thread_runner(void *ptr) {
	...
	for(itr = 0; itr < PUBLISH_COUNT; itr++) {
		sESP_LOGI(TAG, cPayload, "%d", itr + 1);
		...
		if(SUCCESS != rc) {
			ESP_LOGE(TAG, "Error Publishing #%d --> %d\n ", itr, rc);
			usleep(300000);
			rc = aws_iot_mqtt_publish(pClient, INTEGRATION_TEST_TOPIC, strlen(INTEGRATION_TEST_TOPIC), &params);
			rePublishCount++;
			if(SUCCESS != rc) {
				ESP_LOGE(TAG, "Error Publishing #%d --> %d Second Attempt ", itr, rc);
			}
		}
		usleep(300000);
	}
...
int aws_iot_mqtt_tests_multiple_clients() {
	...
	snESP_LOGI(TAG, subClientId, 50, "%s_%d", INTEGRATION_TEST_CLIENT_ID_SUB, rand() % 10000);
	snESP_LOGI(TAG, pubClientId, 50, "%s_%d", INTEGRATION_TEST_CLIENT_ID_PUB, rand() % 10000);

	getcwd(CurrentWD, sizeof(CurrentWD));

	snESP_LOGI(TAG, rootCA, PATH_MAX + 1, "%s/%s/%s", CurrentWD, certDirectory, AWS_IOT_ROOT_CA_FILENAME);
	...
	if(SUCCESS == rc) {
		ESP_LOGI(TAG, "\n## Connect Success. Time sec: %ld, usec: %ld", (long int)connectTime.tv_sec, (long int)connectTime.tv_usec);
	} else {
		ESP_LOGE(TAG, "\n## Connect Failed. error code %d", rc);
		return -1;
	}
	...
	if(SUCCESS == rc) {
		ESP_LOGI(TAG, "## Connect Success. Time sec: %ld, usec: %ld", (long int)connectTime.tv_sec, (long int)connectTime.tv_usec);
	} else {
		ESP_LOGE(TAG, "## Connect Failed. error code %d", rc);
		return -1;
	}
	...
	if(percentOfRxMsg >= RX_RECEIVE_PERCENTAGE && rxMsgBufferTooBigCounter == 0 && rxUnexpectedNumberCounter == 0) {
		ESP_LOGI(TAG, "\nSuccess: %f \%", percentOfRxMsg);
		ESP_LOGI(TAG, "Published Messages: %d , Received Messages: %d ", PUBLISH_COUNT, rxMsgCount);
		ESP_LOGI(TAG, "QoS 1 re publish count %d", rePublishCount);
		ESP_LOGI(TAG, "Connection Attempts %d", connectCounter);
		test_result = 0;
	} else {
		ESP_LOGE(TAG, "\nFailure: %f", percentOfRxMsg);
		ESP_LOGI(TAG, "\"Received message was too big than anything sent\" count: %d\n", rxMsgBufferTooBigCounter);
		ESP_LOGI(TAG, "\"The number received is out of the range\" count: %d\n", rxUnexpectedNumberCounter);
		test_result = -2;
	}
	return test_result;
}
