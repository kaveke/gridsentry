...
void printBuffer(unsigned char *buffer, size_t len) {
	...
	ESP_LOGI(TAG, "\n--");
	for(i = 0; i < len; i++) {
		ESP_LOGI(TAG, "%d: %c, %d", (uint32_t)i, buffer[i], buffer[i]);
	}
	ESP_LOGI(TAG, "\n--");
}
...
bool isConnectTxBufFlagCorrect(IoT_Client_Connect_Params *settings, ConnectBufferProofread *readRes) {
	...
	for(i = 0; i < 8; i++) {
		if(myByte[i] != (unsigned char) (((readRes->ConnectFlag) >> (7 - i)) & 0x01)) {
			ESP_LOGI(TAG, "ex %x ac %x", (unsigned char) (((readRes->ConnectFlag) >> (7 - i)) & 0x01) + '0', myByte[i]);
			ret = false;
			break;
		}
	}
...
bool isConnectTxBufPayloadCorrect(IoT_Client_Connect_Params *settings, unsigned char *payloadBuf) {
	...
	if (0 == strlen(pUsernameTemp)) {
		snESP_LOGI(TAG, pUsernameTemp, SDK_METRICS_LEN, SDK_METRICS_TEMPLATE,
...
void printPrfrdParams(ConnectBufferProofread *params) {
	unsigned int i;
	ESP_LOGI(TAG, "\n----------------");
	ESP_LOGI(TAG, "PacketType: %x", params->PacketType);
	ESP_LOGI(TAG, "RemainingLength: %u", params->RemainingLength);
	ESP_LOGI(TAG, "ProtocolLength: %u", params->ProtocolLength);
	ESP_LOGI(TAG, "ProtocolName: ");
	for(i = 0; i < params->ProtocolLength; i++) {
		ESP_LOGI(TAG, "%c", params->ProtocolName[i]);
	}
	ESP_LOGI(TAG, "");
	ESP_LOGI(TAG, "ProtocolLevel: %u", params->ProtocolLevel);
	ESP_LOGI(TAG, "ConnectFlag: %x", params->ConnectFlag);
	ESP_LOGI(TAG, "KeepAliveInterval: %u", params->KeepAlive);
	ESP_LOGI(TAG, "----------------");
}
