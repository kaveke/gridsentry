...
void genericCallback(const char *pJsonStringData, uint32_t JsonStringDataLen, jsonStruct_t *pContext) {
	ESP_LOGI(TAG, "\nkey[%s]==Data[%.*s]", pContext->pKey, JsonStringDataLen, pJsonStringData);
}

void nestedObjectCallback(const char *pJsonStringData, uint32_t JsonStringDataLen, jsonStruct_t *pContext) {
	ESP_LOGI(TAG, "\nkey[%s]==Data[%.*s]", pContext->pKey, JsonStringDataLen, pJsonStringData);
	snESP_LOGI(TAG, receivedNestedObject, 100, "%.*s", JsonStringDataLen, pJsonStringData);
}
...
TEST_GROUP_C_SETUP(ShadowDeltaTest) {
	...
	snESP_LOGI(TAG, shadowDeltaTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, SHADOW_DELTA_UPDATE, AWS_IOT_MY_THING_NAME);
}
...
TEST_C(ShadowDeltaTest, DeltaNestedObject) {
	...
	snESP_LOGI(TAG, deltaJSONString, 100, "{\"state\":{\"delta\":{\"%s\":%s}},\"version\":1}", nestedObjectHandler.pKey,
...
TEST_C(ShadowDeltaTest, DeltaVersionIgnoreOldVersion) {
	...
	snESP_LOGI(TAG, receivedNestedObject, 100, " ");
	snESP_LOGI(TAG, deltaJSONString, 100, "{\"state\":{\"delta\":{\"%s\":%s}},\"version\":2}", nestedObjectHandler.pKey,
			sentNestedObjectData);
	params.payloadLen = strlen(deltaJSONString);
	params.payload = deltaJSONString;
	params.qos = QOS0;
	...
	snESP_LOGI(TAG, receivedNestedObject, 100, " ");
	snESP_LOGI(TAG, deltaJSONString, 100, "{\"state\":{\"delta\":{\"%s\":%s}},\"version\":3}", nestedObjectHandler.pKey,
			sentNestedObjectData);
	params.payloadLen = strlen(deltaJSONString);
	params.payload = deltaJSONString;
	params.qos = QOS0;
