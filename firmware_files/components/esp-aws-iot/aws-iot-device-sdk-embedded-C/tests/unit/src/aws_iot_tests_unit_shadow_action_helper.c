...
static void topicNameFromThingAndAction(char *pTopic, const char *pThingName, ShadowActions_t action) {
	...
	snESP_LOGI(TAG, pTopic, 100, "$aws/things/%s/shadow/%s", pThingName, actionBuf);
}
...
TEST_GROUP_C_SETUP(ShadowActionTests) {
	...
	snESP_LOGI(TAG, cPayload, 100, "%s : %d ", "hello from SDK", 0);
...
TEST_C(ShadowActionTests, UpdateTheJSONDocument) {
	...
	snESP_LOGI(TAG, expectedUpdateRequestJson, SIZE_OF_UPDATE_DOCUMENT,
	...
	ResetTLSBuffer();
	snESP_LOGI(TAG, jsonFullDocument, 200, "%s", "");
...
TEST_C(ShadowActionTests, SubscribeToAcceptedRejectedOnGet) {
	...
	snESP_LOGI(TAG, LastSubscribeMessage, lastSubscribeMsgLen, "No Message");
	secondLastSubscribeMsgLen = 11;
	snESP_LOGI(TAG, SecondLastSubscribeMessage, secondLastSubscribeMsgLen, "No Message");
	...
	snESP_LOGI(TAG, topicName, topicNameLen + 1u, "%s", &(TxBuffer.pBuffer[4])); // Added one for null character
...
TEST_C(ShadowActionTests, UnSubscribeToAcceptedRejectedOnGetTimeout) {
	...
	snESP_LOGI(TAG, jsonFullDocument, 200, "aa");
	lastSubscribeMsgLen = 11;
	snESP_LOGI(TAG, LastSubscribeMessage, lastSubscribeMsgLen, "No Message");
	secondLastSubscribeMsgLen = 11;
	snESP_LOGI(TAG, SecondLastSubscribeMessage, secondLastSubscribeMsgLen, "No Message");
...
TEST_C(ShadowActionTests, UnSubscribeToAcceptedRejectedOnGetTimeoutWithSticky) {
	...
	snESP_LOGI(TAG, jsonFullDocument, 200, "timeout");
	lastSubscribeMsgLen = 11;
	snESP_LOGI(TAG, LastSubscribeMessage, lastSubscribeMsgLen, "No Message");
	secondLastSubscribeMsgLen = 11;
	snESP_LOGI(TAG, SecondLastSubscribeMessage, secondLastSubscribeMsgLen, "No Message");
...
TEST_C(ShadowActionTests, InvalidInboundJSONInGetResponse) {
	...
	snESP_LOGI(TAG, jsonFullDocument, 200, "NOT_VISITED");
...
TEST_C(ShadowActionTests, AcceptedSubFailsGetRequest) {
	...
	snESP_LOGI(TAG, jsonFullDocument, 200, "NOT_SENT");
...
TEST_C(ShadowActionTests, NoClientTokenForShadowAction) {
	...
	snESP_LOGI(TAG, getRequestJson, TEST_JSON_SIZE, "{}");
	snESP_LOGI(TAG, jsonFullDocument, 200, "NOT_VISITED");
...
TEST_C(ShadowActionTests, IsReceivedJsonValid)
{
	...
	snESP_LOGI(TAG, getRequestJson, TEST_JSON_SIZE, TEST_JSON_RESPONSE_FULL_DOCUMENT);	
