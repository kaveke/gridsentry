...
static uint16_t iot_tls_mqtt_copy_string_from_message(char *dest, const unsigned char *msgBuffer, size_t startPos) {
	...
	snESP_LOGI(TAG, dest, length + 1u, "%s", &(msgBuffer[startPos + 2])); // Added one for null character
	return length;
}
...
IoT_Error_t iot_tls_write(Network *pNetwork, unsigned char *pMsg, size_t len, Timer *timer, size_t *written_len) {
	...
	*written_len = len;
	...
	if((firstPacketByte == 0x82 ? true : false)) {
		snESP_LOGI(TAG, SecondLastSubscribeMessage, lastSubscribeMsgLen + 1u, "%s", LastSubscribeMessage);
