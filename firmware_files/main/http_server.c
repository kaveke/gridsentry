...
static void http_server_fw_update_reset_timer(void)
{
    if(g_fw_update_status == OTA_UPDATE_SUCCESSFUL)
    {
        ...
        // Give the web apge a chance to receive an acknowledge back and initialise the timer
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    if(flash_successful) 
    {
        http_server_monitor_send_message(HTTP_MSG_OTA_UPDATE_SUCCESFUL);
    }
    else{
 ...
 * and handles the receiving the SSID and password entered by the user
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_wifi_connect_json_handler(httpd_req_t *req)
{
    ...
    if(len_ssid > 1)
    {
        ...
        {
            ESP_LOGI(TAG, "http_server_wifi_connect_json_jandler: Found header => my-connect-ssid: %s", ssid_str);
        }
    }
    ...
    if(len_pass > 1)
    {
        ...
        {
            ESP_LOGI(TAG, "http_server_wifi_connect_json_jandler: Found header => my-connect-pwd: %s", pass_str);
        }
    }
    ...
    const char *resp = "Connection request received";
...
{
    ESP_LOGI(TAG, "wifiDisconnect.json handler - Request received");

    wifi_app_send_message(WIFI_APP_MSG_USER_REQUESTED_STA_DISCONNECT);
    ESP_LOGI(TAG, "Disconnect message sent to WiFi app");

    // Respond with a JSON message
    const char *resp = "{\"status\":\"disconnected\"}";
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, resp, strlen(resp));
