...
esp_err_t app_nvs_save_sta_creds(void)
{
    ...
    if (wifi_sta_config)
    {
        ...
        // SECURITY PATCH: Redacted password from logs
        ESP_LOGI(TAG, "app_nvs_save_sta_creds: wrote wifi_sta_config: Station SSID: %s Password: [REDACTED]", wifi_sta_config->sta.ssid);
...
bool app_nvs_load_sta_creds(void)
{
    ...
    if (nvs_open(app_nvs_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK)
    {
        ...
        if(esp_err != ESP_OK)
        {
            ...
            ESP_LOGE(TAG, "app_nvs_load_sta_creds: (%s) no station SSID found in NVS", esp_err_to_name(esp_err));
            return false;
        }
        ...
        {
            free(wifi_config_buff);
            ESP_LOGE(TAG, "app_nvs_load_sta_creds: (%s) no station Password found in NVS", esp_err_to_name(esp_err));
            return false;
        }
        ...
        // SECURITY PATCH: Redacted password from logs and used ESP_LOGI instead of printf
        ESP_LOGI(TAG, "app_nvs_load_sta_creds: SSID: %s Password: [REDACTED]", wifi_sta_config->sta.ssid);
...
esp_err_t app_nvs_clear_sta_creds(void)
{
    ...
    if(esp_err != ESP_OK)
    {
        ESP_LOGE(TAG, "app_nvs_clear_sta_creds: Error (%s) opening NVS handle!", esp_err_to_name(esp_err));
        return esp_err;
    }
    ...
    if(esp_err != ESP_OK)
    {
        ESP_LOGE(TAG, "app_nvs_clear_sta_creds: Error (%s) erasing station mode credentials!", esp_err_to_name(esp_err));
        return esp_err;
    }
    ...
    if(esp_err != ESP_OK)
    {
        ESP_LOGE(TAG, "app_nvs_clear_sta_creds: Error (%s) NVS commit!", esp_err_to_name(esp_err));
        return esp_err;
    }
    nvs_close(handle);
